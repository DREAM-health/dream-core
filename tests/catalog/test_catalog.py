...
Tests for the Test Catalog:
  - Unit CRUD
  - TestPanel CRUD + soft-delete
  - TestDefinition CRUD with nested reference ranges
...
class TestDefinitionCRUD:
    ...
    def test_create_test_with_reference_ranges(
        ...
        assert set(ranges.values_list("sex", flat=True)) == {"male", "female"}
    ...
    def test_filter_by_specimen_type(self, lab_analyst_client: APIClient) -> None:
        ...
        results = resp.json()["results"]
        assert all(t["specimen_type"] == "urine" for t in results)
    ...
    def test_reference_range_validation_low_gte_high(
        ...
        payload = self._payload(
            ...
            code="BADRANGE",
            reference_ranges=[
                {"sex": "any", "low_normal": "50.0", "high_normal": "10.0"}
            ],
        )
        resp = lab_manager_client.post(TESTS_URL, payload, format="json")
        assert resp.status_code == status.HTTP_400_BAD_REQUEST
...
class TestResultInterpretation:
    ...
    def test_uses_sex_specific_range(self, lab_analyst_client: APIClient) -> None:
        test = self._create_test_with_ranges()
